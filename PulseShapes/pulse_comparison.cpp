...
void pulse_comparison(){

  TCanvas *canvas = new TCanvas("Pulse Shape Comparison","Pulse Shape Comparison");
  TFile *template_file = new TFile("/afs/cern.ch/user/k/kyee/work/pulsetest/template_new_7290.root");
  TProfile *mean_waveform = (TProfile*) template_file->Get("mean waveform - C3 - 100 GeV - 6*6 mm^2 at center"); //Note: This title MUST align with what the template generator set it to
  TProfile *interpolated_mean_waveform = (TProfile*) template_file->Get("interpolated mean waveform - C3 - 100 GeV - 6*6 mm^2 at center");

  TFile *template_file2 = new TFile("/afs/cern.ch/user/k/kyee/work/pulsetest/template_new_7416.root");
  TProfile *mean_waveform2 = (TProfile*) template_file2->Get("mean waveform - D3 - 100 GeV - 6*6 mm^2 at center");
  TProfile *interpolated_mean_waveform2 = (TProfile*) template_file2->Get("interpolated mean waveform - D3 - 100 GeV - 6*6 mm^2 at center");
...
//  cout << "tmax of D3 is " << tmax2 << endl; //Both are correct
  ...
  TH1F *new_interpolation = new TH1F("interpolated mean waveform - C3 - 100 GeV - scaled","interpolated mean waveform - C3 - 100 GeV - scaled",nTempBins,-0.125 - tmax,937.375 - tmax);
  TH1F *new_interpolation2 = new TH1F("interpolated mean waveform - D3 - 100 GeV - scaled","interpolated mean waveform - D3 - 100 GeV - scaled",nTempBins,-0.125 - tmax2,937.375 - tmax2);
  TH1F *new_int_diff = new TH1F("interpolated mean waveform - C3/D3 Difference","interpolated mean waveform - C3/D3 Difference",nTempBins,-0.125 - tmax,937.375 - tmax);
...
//  for (int i = 0;i < nTempBins;i++) new_int_diff[i] = TMath::Abs( new_interpolation->GetBinContent(i) - new_interpolation2->GetBinContent(i) ) ;
  ...
  for (int i = 0;i < nTempBins - TMath::Abs(offset) ;i++) new_int_diff->SetBinContent(i, TMath::Abs( new_interpolation->GetBinContent(i - offset) - new_interpolation2->GetBinContent(i) ) );

TGraph *graph = new TGraph(nsamples, x, y);
TGraph *graph2 = new TGraph(nsamples, x2, y2);
TGraph *grdiff = new TGraph(nsamples, xave, ydiff);

graph->GetXaxis()->SetRangeUser(-25,40);
graph2->GetXaxis()->SetRangeUser(-25,40);
grdiff->GetXaxis()->SetRangeUser(-25,40);
new_interpolation->GetXaxis()->SetRangeUser(-25,45);
new_interpolation2->GetXaxis()->SetRangeUser(-25,45);
new_int_diff->GetXaxis()->SetRangeUser(-25,45);
